...
const NOMINAL: ElementAppearance = { color: ELEMENT_COLOURS.nominal, scale: 1 };
...
export function elementAppearance(
  ...
  if (isSelected) return { color: SELECTED_ELEMENT_COLOUR, scale: 1.35 };
  ...
  if (el.health === "critical") return { color: ELEMENT_COLOURS.critical, scale: 1.35 };
  if (el.health === "offline") return { color: ELEMENT_COLOURS.offline, scale: 0.85 };
  if (el.health === "degraded" && metricMode === "states") {
    return { color: ELEMENT_COLOURS.degraded, scale: 1.15 };
  }

  switch (metricMode) {
    case "gain":
      return { color: linearRampColor(el.amplitude, GAIN_RAMP_RANGE[0], GAIN_RAMP_RANGE[1], GAIN_RAMP[0], GAIN_RAMP[1]), scale: 1 };
    case "phase":
      return { color: cyclicPhaseColor(el.phase), scale: 1 };
    case "temp":
      return { color: linearRampColor(el.tempC, TEMP_RAMP_RANGE[0], TEMP_RAMP_RANGE[1], TEMP_RAMP[0], TEMP_RAMP[1]), scale: 1 };
    case "states":
    default:
      return NOMINAL;
  }
